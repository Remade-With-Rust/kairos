...
**Unit**: `__NAME__` — package (facade + `no_std` core)
**Tier**: __TIER__ — an RTOS component: the kernel is the trusted computing base of every firmware above it, and every library here parses bytes from a wire, a store or a bus
...
**Audit depth**: survey
**Audited**: __DATE__ by kairos (scaffold pass) · **Next review**: the first milestone with a kill test
...
|---|---|---|---|---|---|
| __DATE__ | survey | kairos (scaffold pass) | 7 / 0 / 28 | 5 | first pass, at stamp time; every Completed row names a file that exists |
