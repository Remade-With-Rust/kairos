...
## The build fact (__DATE__)

| gate | result |
|---|---|
| `cargo check --workspace` on the host | passes at scaffold |
| `cargo check -p __NAME__-core --no-default-features` and `--features alloc` on `thumbv7em-none-eabihf`, `thumbv8m.main-none-eabihf`, `riscv32imac-unknown-none-elf`, `riscv32imafc-unknown-none-elf` | passes at scaffold (`kairos check`) |
