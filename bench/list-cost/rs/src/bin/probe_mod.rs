//! VARIANT: exact-size array and a MODULO instead of a mask, so no
...
impl Lists {
    ...
    fn at(&self, link: u16) -> &Node {
        &self.nodes[(link as usize) % SLOTS]
    }

    #[inline]
    fn at_mut(&mut self, link: u16) -> &mut Node {
        &mut self.nodes[(link as usize) % SLOTS]
    }
