...
4. **K3 — silicon and an emulator** (bench + QEMU). `-cortex-m` on QEMU
...
5. **K4 — the heaps and the allocator.** `heap_1`, `heap_4`, `heap_5` with
...
6. **K5 — the Janus joint** (bench). `rusty_rtos_port-riscv` and `-xtensa`
...
7. **K6 — the C ABI and the unmodified corpus.** `rusty_rtos-capi` +
...
8. **K7 — the libraries.** In this order, each its own package with its own
...
9. **K8 — SMP, MPU, tickless idle, stats, hardening, 1.0.** `configNUMBER_OF_CORES
...
10. **K9 — the rest of the portfolio** (`pkcs11`, `cellular`, `fat`, `posix`,
...
| **K2 IPC + timers** | the remaining demo scenarios trace-identical; Kani harnesses for the CBMC proof list pass; no-panic property test; mutants score ledgered | **11 of 18 passed 2026-09-09.** The corpus is thirteen scenarios in all (the other two are K1's) and 11,328,945 lines identical at 100 000 ticks. In the kernel: the from-ISR surface, queue sets, real queue lock counts, task notifications, stream and message buffers with their own arena. Of the seven not passed, **four are blocked above the kernel** — `IntQueue` needs nested interrupts this port does not have; `MessageBufferDemo` and `StreamBufferDemo` starve sim contract v1 (a task that polls without entering a critical section stops the clock); `QueueSet` seeds its PRNG from a stack address; `MessageBufferAMP` needs its own oracle binary. **Two need kernel work:** `TimerDemo` (software timers and the daemon) and `EventGroupsDemo` (event groups, and the timers under them). All four blockers and their evidence are ledger rows |
