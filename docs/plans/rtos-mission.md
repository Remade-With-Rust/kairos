...
| **K2 IPC + timers** | the remaining demo scenarios trace-identical; Kani harnesses for the CBMC proof list pass; no-panic property test; mutants score ledgered | **13 of 18 passed 2026-09-09.** The corpus is fifteen scenarios in all (the other two are K1's) and 12,688,209 lines identical at 100 000 ticks. In the kernel: the from-ISR surface, queue sets, real queue lock counts, task notifications, stream and message buffers with their own arena, the software timers with their daemon, and event groups. Of the five not passed, **every one is blocked above the kernel** — `IntQueue` needs nested interrupts this port does not have; `MessageBufferDemo` and `StreamBufferDemo` starve sim contract v1 (a task that polls without entering a critical section stops the clock); `QueueSet` seeds its PRNG from a stack address; `MessageBufferAMP` needs its own oracle binary. All of them and their evidence are ledger rows |
