...
| **coreSNTP** | `rusty_rtos_sntp` | the SNTPv4 packet codec and clock arithmetic ✅ (the client state machine is next) | — | NTS on a chip in v1; the full NTPv4 engine, which is the house `rusty_time`'s job |
...
| 3 | **`rusty_rtos_sntp`** ✅ **serializer done 2026-09-16; the client state machine is next** | **160 trace lines agree with `core_sntp_serializer.c`** across 75 calls, including both sides of the 2036 era wrap and the half-era tie from both directions. The trace carries its own inputs, so there is no second copy of the C's table to drift | its hardest defect is DATED — an era comparison that is wrong is correct until 2036 and silently wrong afterwards, which is what a differential is for and what no amount of testing-against-today would find |
