...
| **K3 silicon + QEMU** | the full corpus check task passes one hour on M3-qemu, RV32-qemu and a C6; context switch / tick / latency cycle rows vs the C demo **from the C6, not from QEMU**; flash + RAM decomposition | **PART DONE — and the measurement half is done except for one purchase.** **Closed:** the corpus is byte-identical to C FreeRTOS on **four** targets (host, ARMv7-M, RV32 and a XIAO ESP32-S3, the last a real part); the hour is closed on the host and on silicon; flash and RAM are both compared against the C map (**15,950 B against 13,924, 1.15x**; per task **207 B against 596 B**); and **all three cycle rows exist on silicon** — tick **54**, switch **166**, ISR-wake-to-task **430** cycles, re-measured 2026-09-21 after the numbers this row used to carry (131 / 623 / 949) were found to have been measured through a harness defect. **And the C arm exists as a WORK row**: `bench/tick-work/run.sh` runs FreeRTOS **V11.3.1 out of the pinned oracle, unmodified**, against Kairos on one machine under one instrument — tick **15 against 56**, scheduler selection **27 against 79**, both against us and published as findings; but a whole cooperative switch is **110 against 109, parity**, because the two kernels put their weight in opposite halves. **Open, and this is the whole list.** (1) **A C6** — a second chip and a second architecture of record, and the same purchase unblocks K5's kill test. (2) **Cycles with a C arm beside them**, which needs an **ESP-IDF install (~2 GB)** rather than hardware: the pinned oracle carries its own S3-capable Xtensa port and the S3 compiler is already on the box, so the blocker is the SDK, not the part. Owner's call, recorded unbought. (3) **`AbortDelay` FAILS the hour, bisected to a 1,000-tick bracket**: it passes at **220,000** ticks and fails at **221,000**, on RV32, reproducibly. `pass=false runaway=false` — it reaches the full length every time, so it neither hangs nor runs away; its check task declines to pass. Which of `still_running`'s three conditions fires (controlling cycles stalled, blocking cycles stalled, or `error` set) is **not yet determined and not guessed at**; printing the three separately at the failing length is a one-line change and a single run. This is a liveness failure two orders of magnitude out from its known conformance gap at the pinned length, and the two are not assumed to share a cause. (4) **M3 has had none of it.** RV32's hour is **24 of 25** and fully re-measured: the eighteen were re-run under the corrected harness — the one that measured them originally counted any line beginning with a scenario's name as a pass and so could not tell `ok` from `FAIL` — and **all eighteen pass**, with six of the seven never-soaked scenarios passing beside them. M3's eighteen still rest on the broken harness and it has never seen the seven. *(A timing discrepancy is open and deliberately unexplained: 8 minutes against a recorded 58 for the same eighteen. Tick counts rule out a shortened build; beyond that it is not guessed at.)* (5) **`StreamBufferDemo` diverges on both emulators** at the pinned length — every counter and the line count matching, 194 bytes of trace text differing — while **passing** the hour, which is what a trace-text defect looks like rather than a scheduling one. **Refuted and recorded so it is not rebuilt:** a host callgrind stand-in for the C arm. FreeRTOS's tick self cost is 2.7% of its inclusive and ours is 13.1%, so a self-cost ratio measures how two codebases factor a tick, not what a tick costs — and perfect work parity did not save it. Every number, with its method line: **`docs/LEDGER.md`**. |
