...
| **K2 IPC + timers** | the remaining demo scenarios trace-identical; Kani harnesses for the CBMC proof list pass; no-panic property test; mutants score ledgered | **11 of 18 passed 2026-09-09**, `IntQueue` out of scope, 6 open. The corpus is thirteen scenarios in all (the other two are K1's) and 11,328,945 lines identical at 100 000 ticks. In the kernel: the from-ISR surface, queue sets, real queue lock counts, task notifications, stream and message buffers. Open: `QueueSet`, `TimerDemo`, `EventGroupsDemo`, `StreamBufferDemo`, `MessageBufferDemo`, `MessageBufferAMP`, and the three gates that are not scenarios |
