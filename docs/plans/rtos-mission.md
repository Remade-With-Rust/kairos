...
| **K3 silicon + QEMU** | the full corpus check task passes one hour on M3-qemu, RV32-qemu and a C6; context switch / tick / latency cycle rows vs the C demo **from the C6, not from QEMU**; flash + RAM decomposition | **PART DONE — and the measurement half is done except for one purchase.** **Closed:** the corpus is byte-identical to C FreeRTOS on **four** targets (host, ARMv7-M, RV32 and a XIAO ESP32-S3, the last a real part); the hour is closed on the host and on silicon; flash and RAM are both compared against the C map (**15,950 B against 13,924, 1.15x**; per task **207 B against 596 B**); and **all three cycle rows exist on silicon** — tick **54**, switch **166**, ISR-wake-to-task **430** cycles, re-measured 2026-09-21 after the numbers this row used to carry (131 / 623 / 949) were found to have been measured through a harness defect. **And the C arm exists as a WORK row**: `bench/tick-work/run.sh` runs FreeRTOS **V11.3.1 out of the pinned oracle, unmodified**, against Kairos on one machine under one instrument — tick **15 against 56**, scheduler selection **27 against 79**, both against us and published as findings; but a whole cooperative switch is **110 against 109, parity**, because the two kernels put their weight in opposite halves. **Open, and this is the whole list.** (1) **A C6 — and nothing else.** Both cells now exist and build: `rusty_rtos_demo/firmware/esp32c6-corpus` (the hour, and it builds with `--features soak` too) and `rusty_rtos_kernel/firmware/esp32c6-cycles` (the cycle rows). **Neither has ever been flashed and neither carries a number**, and both say so in their own banner. The point of writing them unrun is that there was no C6 cell of ANY kind in the family, so a board arriving would have meant writing two cells and finding the toolchain differences before measuring anything; now it means `cargo run --release`. Checked rather than assumed: the cycles binary carries 20 `mcycle` reads in its disassembly and the corpus binary places `.flash.appdesc`, so both instruments survive to the artifact. Two findings fell out — `-nostartfiles` is Xtensa-only and `rust-lld` rejects it on RISC-V, and **a C6 cell builds on STABLE** (no esp toolchain, no `build-std`), making it the only silicon cell CI could build. The same purchase unblocks K5's kill test. (2) **Cycles with a C arm beside them**, which needs an **ESP-IDF install (~2 GB)** rather than hardware: the pinned oracle carries its own S3-capable Xtensa port and the S3 compiler is already on the box, so the blocker is the SDK, not the part. Owner's call, recorded unbought. (3) **`AbortDelay` FAILS the hour, bisected to a 1,000-tick bracket**: it passes at **220,000** ticks and fails at **221,000**, on RV32, reproducibly. **and it is NOT a liveness failure, which this row said twice before measuring it.** `tests/abortdelay_hour.rs` prints all three of `still_running`'s conditions: both cycle counters climb the whole way (868 at 220,000 ticks, 872 just past the boundary, **14,904** at the full hour), so neither task ever stops being scheduled. What fires is **`error`** — `prvCheckExpectedTimeIsWithinAnAcceptableMargin`, the scenario catching a block that came back outside its margin. That is a timing-correctness fault, not a scheduling one, and the hour's own claim (every check task still reports running) is in fact SATISFIED here. **And it is now NAMED**: the first failure is `expected 100, blocked 0` at `pc 72` — `prvTestAbortingQueueSend`'s first step, a `queue_send` on the one-deep queue that must block for 100 ticks and time out. It returns **immediately**, so the queue was not full when the test required it. Not a near miss against the 7-tick margin: the call did not block at all. Both tasks cycle **868 times cleanly** and fail on about the 869th, which is the shape of a slow drift rather than a missing case. **And 'the queue was not full' turned out to be MY misreading.** The probe could not read the queue at all: `queue_messages_waiting` is refused — `Gone` at the passing length, `InvalidHandle` at the failing ones — so **the handle does not name a live queue**. More to the point, the scenario cannot notice: `pc 72`'s match arm is `_ => self.checked(..)`, which catches `Err(..)` as well as a completed send, so a **refused** send reaches the margin check exactly as an instant one does and presents as `blocked 0`. The exact boundary is now **220,387 ticks, passing at 220,386** — one tick, at 870 controlling cycles, on the host, consistent with both emulators. **ROOT CAUSE FOUND: `queue_create` is refused after ~870 create/delete cycles.** The scenario creates and deletes its queue every cycle; `unwrap_or_default()` turns the refusal into a default handle; `queue_send` on it returns `Err(InvalidHandle)`; `pc 72`'s `_` arm treats that as a completed send; the margin check sees 0 ticks against 100 and flags an error. **Six layers between cause and symptom**, which is why the first five readings were wrong, three of them mine. **ESTABLISHED, and it is a LEAK — a kernel defect shipped in 0.2.0.** `tests/queue_slot_leak.rs` samples free queue capacity after runs of several lengths, on a scenario that deletes everything it creates: **11 free at 1,000 and 100,000 ticks, 10 at 200,000, 5 at 210,000, 2 at 215,000, and ZERO by 219,000**. Capacity should not move; it goes to zero. Two things the shape says. **Exhaustion precedes the failure** — capacity is already zero at 220,386, where the scenario still passes, so the failure's timing is set by when it next needs to CREATE, which is why bisecting to a single tick found the symptom's edge and not the defect's. And **the rate is not uniform**, flat for ~400 cycles then away, which is unexplained. **And the simple mechanism is REFUTED.** `tests/which_resource_leaks.rs` runs each pair alone on a fresh kernel for forty rounds — more than three times the twelve-slot capacity — and **every one reclaims perfectly**: queue, binary semaphore, event group and stream buffer all end at 12 of 12. So capacity really does fall under `AbortDelay` and **no create/delete pair leaks by itself**; both are measured and they are not in conflict. The exhaustion needs something the scenario does that a bare loop does not. **That suspect is REFUTED too, and so is the one after it.** A task parked on the queue does not stop reclamation — block→delete, block→**abort**→delete and the timed-out control all hold 11 of 11 over forty rounds. Nor does the scenario skip a delete: counters show **108 created and 108 deleted** by 219,000 ticks, exact, and all four objects it builds have a matching delete. **Seven hypotheses, seven refutations.** What stands is measured and bounded: capacity falls 11→0 over ~108 passes, no pair leaks alone, a waiter does not block reclaim on a queue, and every create is matched. The mechanism is **unknown**, and that named probe has now been RUN and refuted too: the waiter test covers all four surfaces and **all six shapes hold 11 of 11** — queue, semaphore, event group and stream buffer, with and without an abort. **Eight hypotheses, eight refutations.** Every isolated reproduction of what the scenario does reclaims correctly and the scenario still exhausts, which points away from the create/delete APIs and towards how the RUNNER drives them. **That instrumentation was built, and it FOUND IT.** Stepping the scenario while sampling capacity accelerated the exhaustion twelve-fold — 11 slots in 9 passes against 108 — because the sampler does the one thing no earlier probe did: **it holds many objects at once**. Isolated (`tests/batch_vs_single.rs`): create-one-delete-one loses **0 of 12** over ten rounds; fill-to-refusal-then-delete-all loses **4 of 12**, in either order. **And the source says it plainly, which nine readings of BEHAVIOUR did not: `queue_delete` does not free item storage.** `new_queue` bump-allocates from `slots_used`; `slots_used` appears four times in the crate — declared, zeroed, read, written — and is **never decremented**. The descriptor comes back and the storage never does. The arithmetic confirms it to the unit against three measurements taken before the mechanism was known: `SLOTS = 128`, single 100 creates → capped at 12 descriptors (**measured 12**), batch 120 creates → 8 left (**measured 8**), `AbortDelay` ~1.18 slots/pass → exhausted at ~108 passes (**measured 108**). **Any application that creates and deletes data-carrying queues exhausts `SLOTS` permanently**; semaphores, mutexes and event groups are unaffected because they carry no data. Shipped in 0.2.0. **FIXED, and BOTH emulator hours are now 25 of 25 — confirmed on each, not inferred from one.** Three lines in `queue_delete` return the extent to the bump pointer when it is the last allocation — the same move `give_bytes` already makes for the byte arena, whose comment says *'a bump allocator would run out in a few hundred ticks'*; the identical argument applied here and had never been made. `AbortDelay` now passes the hour on **RV32 and on Cortex-M3, identical field for field** (3,600,020 ticks, 152,702 yields, 3,948,990 exits, 4,556,276 lines on both) and on the host (14,205 cycles, `error` clear), and the batch leak is gone (12 of 12, both drain orders). **Conformance is untouched and it was proved, not assumed**: `AbortDelay` is 2,549 lines identical with the same counters, and `conform --all` is **22 identical, exit 0, zero divergences**. **And that known limit is now closed too**: the slot arena has the full `free_blocks` treatment — `free_slots` bounded by `QUEUES`, first-fit `take_slots`, coalescing `give_slots`, and `new_queue` returning the slots if the descriptor arena refuses after they were taken (a leak the fix would otherwise have introduced). The shape that separates partial from complete — create three, delete the middle first — now holds **12 of 12** where the end-of-arena fix would have stranded it. Proved against the corpus: **22 identical, exit 0, zero divergences**, counters unchanged, the kernel's own 85+2+5+8 tests green including the Kani proofs, and the hour passing on both emulators identical field for field. *(The first version of that test measured its baseline with the operation under test and reported 12 of 12; rebuilt on fresh kernels it reports 4. Real, and a third the size first claimed.)* Still open: the exact rule, whether this is all of `AbortDelay`'s exhaustion, and which arena. *(A unit error corrected on the way: the queue is created **108** times by 219,000 ticks, not 870 — `controlling_cycles` counts sub-tests and there are eight per pass, so every earlier '~870 cycles' here counted the wrong thing.)* Two lesser defects found on the way and both fixed in the scenario: `unwrap_or_default()` swallowing a fallible create, and `_` as a match arm over a `Result`, which makes a refused call indistinguishable from a completed one and can mask a kernel defect indefinitely. (4) **DONE — both emulator hours are measured on the whole 25-scenario corpus and agree exactly: 24 of 25 on each**, `AbortDelay` the only failure on either, RV32 in 8 minutes and M3 in 13. This replaces a previously-recorded 18/18 on each, which covered a smaller corpus and was counted by a harness that could not tell `ok` from `FAIL`. *(A timing discrepancy is open and deliberately unexplained: 8 minutes against a recorded 58 for the same eighteen. Tick counts rule out a shortened build; beyond that it is not guessed at.)* (5) **`StreamBufferDemo` diverges on both emulators** at the pinned length — every counter and the line count matching, 194 bytes of trace text differing — while **passing** the hour, which is what a trace-text defect looks like rather than a scheduling one. **Refuted and recorded so it is not rebuilt:** a host callgrind stand-in for the C arm. FreeRTOS's tick self cost is 2.7% of its inclusive and ours is 13.1%, so a self-cost ratio measures how two codebases factor a tick, not what a tick costs — and perfect work parity did not save it. Every number, with its method line: **`docs/LEDGER.md`**. |
